...
const PodcastPage = ({data:{sanityPodcast:{title, description, links}}}) => {
  return (
    <Layout>
      ...
      <p className="text-xl my-14">{description} <a className="text-red-800 underline" href="https://feeds.transistor.fm/more-to-the-story-with-dr-andy-miller">View All Episodes</a></p>
      <div className="grid grid-cols-2 gap-5 md:flex md:space-x-5 mb-14">
        {links.map((link) => (
          <a
            ...
            width="200"
          >
            <SanityImage {...link.image} alt={link.url}></SanityImage>
          </a>
        ))}
      </div>
      ...
      <hr className="my-14 border-red-900 border-solid"></hr>
      <p className="text-xl my-14">Listen to an Archive of Captain's Corner. <a className="text-red-800 underline" href="https://feeds.transistor.fm/captains-corner-podcast-archives">View All Episodes</a></p>
